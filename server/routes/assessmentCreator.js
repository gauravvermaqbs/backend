...
router.post("/wisdomAPI", async (req, res) => {
  const { text, formatType, questionFormat,no_of_questions, temperature,language } = req.body;
  // console.log(formatType, questionFormat,no_of_questions, temperature,language);
  const configuration = new Configuration({
    apiKey: process.env.OPENAI_API_KEY,
  });
